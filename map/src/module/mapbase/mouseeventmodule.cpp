...
#include "mouseeventmodule.h"

void MouseEventModule::setMapNode(osgEarth::MapNode * mapNode)
{
    m_mapNode = mapNode;
}

void MouseEventModule::registerMouseMoveCallback(MouseEventModule::MouseMoveCallback callback)
{
    m_mouseMoveCallbacks.push_back(callback);
}

MouseEventModule::MouseEventModule()
{
}
...
bool MouseEventModule::handle(const osgGA::GUIEventAdapter & ea, osgGA::GUIActionAdapter & aa)
{
    if (ea.getEventType() == osgGA::GUIEventAdapter::MOVE)
    {
        ...
        if (terrain && terrain->getWorldCoordsUnderMouse(viewer, x, y, world))
        {
            ...
            double diatance = getFocalLength(viewer, m_mapNode);
            ...
            double elevation = heightAboveMSL;
...
// 计算视点焦距
double MouseEventModule::getFocalLength(osgViewer::Viewer * viewer, osgEarth::MapNode * mapNode)
{
    // 获取相机位置
...
double MouseEventModule::getElevation(osg::ref_ptr<osgEarth::MapNode> mapNode, const osg::Vec3d & worldPos)
{
    // 转换到地球坐标系
...
// 计算比例尺
double MouseEventModule::getScale(osgViewer::Viewer * viewer)
{
    if (!viewer)
        return 1.0;
    osg::Camera * camera = viewer->getCamera();
